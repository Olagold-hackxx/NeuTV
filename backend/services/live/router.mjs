...
export function createLiveRouter(deps) {
  ...
  r.get('/live/telemetry', async () => ok(await service.telemetry()), { auth: 'none' });
  ...
  r.get('/live/comments',  async (req) => ok(await service.comments({ limit: Number(req.query.limit) || 30 })), { auth: 'none' });
  ...
  r.post('/live/reactions', async (req) => ok(await service.react(req.auth, req.body)), { auth: 'optional', limit: { tokens: 120, windowMs: 60_000 } });
  r.get('/live/reactions',  async () => ok(await service.reactions()), { auth: 'none' });

  r.get('/live/chat/:serverId/:channelId', async (req) => ok(await service.chat(req.params.serverId, req.params.channelId, {
  ...
  r.post('/live/presence',   async (req) => ok(await service.presence(req.auth, req.body)), { auth: 'optional', limit: { tokens: 120, windowMs: 60_000 } });
