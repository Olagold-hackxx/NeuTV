...
test('every declared route is implemented by its service', async () => {
...
test('no service exposes a route the contract does not declare', async () => {
  const app = await build();
...
test('auth levels in the router match the contract exactly', async () => {
  const app = await build();
  for (const declared of ROUTES) {
    const router = app.routers[declared.service];
    const found = router.routes.find((r) => r.method === declared.method && r.pattern === declared.path);
...
test('stream and raw flags match the contract', async () => {
...
test('contract declares a known auth level for every route', async () => {
...
test('every service in the manifest is composed, and vice versa', async () => {
  const app = await build();
  assert.deepEqual([...SERVICES].sort(), Object.keys(app.routers).sort());
  await app.close();
});

test('route patterns are unique per method', async () => {
  const seen = new Set();
