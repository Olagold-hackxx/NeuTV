...
const OUT = join(BACKEND_ROOT, 'services', 'catalog', 'seed', 'catalog.seed.json');
...
const SOURCES = [
  join(REPO_ROOT, 'frontend', 'src', 'data.js'),
  join(REPO_ROOT, 'frontend', 'index.html'),
];
const pickSource = () => {
...
if (isMain) {
  ...
  const seed = buildSeed(readFileSync(source, 'utf8'), source.replace(REPO_ROOT + '/', ''));
