// Segment upload proxy.
//
// Same reasoning as the video upload proxy: the browser must not hold an admin
// bearer token, so the chunk streams through here with the httpOnly cookie
// attached server-side. Segments are small and frequent, so this stays cheap.
...
export async function PUT(
  ...
  const upstream = await fetch(`${API_BASE}/api/v1/admin/live-events/${eventId}/segment${init}`, {
    method: 'PUT',
    headers: {
