...
import { startLiveSession, stopLiveSession } from '@/lib/actions';
...
export function Studio({ event }: { event: LiveSession }) {
    ...
    try {
      ...
      if (event.whipUrl) {
        try {
          ...
          setRecording(true);
          if (!event.isLive) await startLiveSession(event.id, 'whip');
          router.refresh();
      ...
      if (!event.isLive) {
        const res = await startLiveSession(event.id, 'segments');
  ...
  const end = async () => {
    ...
    if (publisherRef.current) {
      ...
      stopTracks();
      setRecording(false);
      await stopLiveSession(event.id);
      router.refresh();
      return;
    }
    recorderRef.current?.state === 'recording' && recorderRef.current.stop();
