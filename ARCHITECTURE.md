...
```
                      ┌──────────────────────────────┐
   browser ──────────▶│  gateway                     │
   src/neutv-api.js   │  sockets, auth, limits, SSE  │
                      └──────────────┬───────────────┘
                                     │ normalized request
              ┌──────────┬───────────┼───────────┬──────────┐
              ▼          ▼           ▼           ▼          ▼
...
| Deterministic (code) | Latent (local Claude Code) |
| --- | --- |
| Seed extraction from `src/data.js` | Grey-band moderation calls |
| Schedule resolution, search ranking | |
| Moderation ruleset, ledger arithmetic | |
| Stage expiry, telemetry, pagination | |

The seed is the clearest case: the content lives in a JS blob in the frontend,
so a script parses it (`scripts/extract-seed.mjs`) rather than anyone retyping
it. It is idempotent and checksummed, so drift is visible.
