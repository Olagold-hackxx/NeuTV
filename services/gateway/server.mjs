...
export function createGateway(options = {}) {
  ...
  const uploadsRoot = options.uploadsRoot || join(ROOT, 'services', 'admin', 'data', 'uploads');
  const staticRoot = options.staticRoot || ROOT;
    ...
    try {
      if (path === '/health' || path === '/api/health') {
        return send(res, 200, {
          ...
          llm: app.services.llm.health().available,
...
if (isMain) {
  ...
  server.listen(port, () => {
    const llm = app.services.llm.health();
    ...
    console.log(`  llm        ${llm.available ? `local Claude Code at ${llm.bin}` : 'unavailable (moderation uses deterministic rules only)'}`);
