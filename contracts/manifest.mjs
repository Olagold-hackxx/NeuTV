...
export const ROUTES = [
  ...
  { service: 'moderation', method: 'GET',  path: '/moderation/health', auth: 'none',     summary: 'Ruleset version and LLM escalation availability.' },

  // --- llm: local Claude Code, never a hosted API --------------------------
  { service: 'llm', method: 'GET',  path: '/llm/health',   auth: 'none',     summary: 'Whether local Claude Code is reachable, and where.' },
  { service: 'llm', method: 'POST', path: '/llm/complete', auth: 'required', summary: 'Run a prompt through local Claude Code.' },
];
